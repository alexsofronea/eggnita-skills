...
- **Show answers to pick, don't make them type.** Render each gap you decide to ask about as a short list of recommended answers — the catalog's own values (product types, stages, access models, channels, and so on) — using your harness's selectable-question tool (in Claude Code, `AskUserQuestion`) rather than an open prompt. Lead with your inferred best guess and mark it *(recommended)* so the user can confirm in one click. Always leave a talk-it-through escape: an explicit "None of these — let's discuss" option alongside the free-text "Other" the tool adds by default, so a pick-list never traps someone whose answer isn't listed. Batch naturally related gaps into one card, and turn on multi-select where the answer isn't exclusive (marketing channels, providers). If the tool isn't available, list the options as plain text the user can answer by label. This is presentation, not a return to the fixed form — you still infer first and surface only the questions that actually change the plan.
